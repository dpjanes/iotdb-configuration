 ...
 *  Parameterized
 */
const get_p = name => (_key, _otherwise) => _.promise((self, done) => {
    const named = `${name}d`;
 ...
 *  Produces: 
 */
const get_keyed = name => (key_from, key_to, otherwise) => _.promise(self => {
    const method = `get_keyed(${name})`;
    const named = `${name}d`;

    assert.ok(name)
    assert.ok(_.is.String(key_from), `${method}: key_from must be a String`)
    assert.ok(_.is.String(key_to), `${method}: key_to must be a String`)

    _.d.set(self, key_to, _.d.get(self[name], key_from, _.is.Undefined(otherwise) ? null : otherwise))
})
