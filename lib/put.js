 ...
 */
const put = name => _.promise.make(self => {
    const method = `put(${name})`;
    const named = `${name}d`;
    ...
    assert.ok(!_.is.Undefined(self[name]), `${method}: self[name='${name}'] is required`)
 ...
 *  Parameterized
 */
const put_p = name => (_key, _value) => _.promise.make((self, done) => {
    const named = `${name}d`;
